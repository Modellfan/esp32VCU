...
bool decode0x2A8(const CANMessage &frame) {
    ...
    const uint16_t rawNewFL = (uint16_t)readBitsLE(frame.data, 0, 16);
    const uint16_t rawNewFR = (uint16_t)readBitsLE(frame.data, 16, 16);
    const uint16_t rawNewRL = (uint16_t)readBitsLE(frame.data, 32, 16);
    const uint16_t rawNewRR = (uint16_t)readBitsLE(frame.data, 48, 16);
...
void encode0x2A8(CANMessage &frame) {
    ...
    const uint16_t rawNewFL = clampU16((int32_t)((params::vehicle.wheel_speed_fl + 1.6338f) / 0.005218f + 0.5f));
    const uint16_t rawNewFR = clampU16((int32_t)((params::vehicle.wheel_speed_fr + 1.6338f) / 0.005218f + 0.5f));
    const uint16_t rawNewRL = clampU16((int32_t)((params::vehicle.wheel_speed_rl + 1.6338f) / 0.005218f + 0.5f));
    const uint16_t rawNewRR = clampU16((int32_t)((params::vehicle.wheel_speed_rr + 1.6338f) / 0.005218f + 0.5f));
    ...
    writeBitsLE(frame.data, 0, 16, rawNewFL);
    writeBitsLE(frame.data, 16, 16, rawNewFR);
    writeBitsLE(frame.data, 32, 16, rawNewRL);
    writeBitsLE(frame.data, 48, 16, rawNewRR);
}
...
bool decode0x1A0(const CANMessage &frame) {
    ...
    params::vehicle.vehicle_reference_speed = readBitsLE(frame.data, 17, 16) * 0.01f;
    params::vehicle.tcs_intervention_slow = readBitsLE(frame.data, 32, 8) * 0.39f;
    params::vehicle.tcs_intervention_fast = readBitsLE(frame.data, 40, 8) * 0.39f;
    params::vehicle.intervention_torque = readBitsLE(frame.data, 48, 8) * 0.39f;
    params::vehicle.lateral_acceleration = readBitsLE(frame.data, 48, 8) * 0.01f - 1.27f;
    return true;
}
...
void encode0x1A0(CANMessage &frame) {
    ...
    const uint16_t rawVRef = clampU16((int32_t)(params::vehicle.vehicle_reference_speed / 0.01f + 0.5f));
    ...
    const uint8_t rawBit48 = clampU8((int32_t)(params::vehicle.intervention_torque / 0.39f + 0.5f));
    ...
    writeBitsLE(frame.data, 17, 16, rawVRef);
    writeBitsLE(frame.data, 32, 8, rawTcsSlow);
    writeBitsLE(frame.data, 40, 8, rawTcsFast);
    writeBitsLE(frame.data, 48, 8, rawBit48);
}
...
bool decode0x00C0(const CANMessage &frame) {
    ...
    const int32_t rawOld = can986::signExtend(readBitsLE(frame.data, 4, 12), 12);
    const int32_t rawNew = can986::signExtend(readBitsLE(frame.data, 0, 16), 16);
...
void encode0x00C0(CANMessage &frame) {
    ...
    writeBitsLE(frame.data, 4, 12, rawOld);
    writeBitsLE(frame.data, 0, 16, rawNew);
}
